...
void DataDistribution(double* pMatrix, double* pProcRows, double* pVector,
    int Size, int RowNum) {
    double* pInitVector = new double[Size];
    ...
    int* pVSendNum; // Number of vector elements sent to the process
    int* pVSendInd; // Index of the first vector data element sent to the process
    int VRestRows = Size; // Number of rows, that haven�t been distributed yet
    //MPI_Bcast(pVector, Size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    // Alloc memory for temporary objects
    pSendInd = new int[ProcNum];
    pSendNum = new int[ProcNum];
    pVSendInd = new int[ProcNum];
    pVSendNum = new int[ProcNum];
    double* pTranspousedMatrix = new double[Size * Size];
    ...
    for (int i = 1; i < ProcNum; i++) {
        ...
        pVSendNum[i] = RowNum;
        pVSendInd[i] = pVSendInd[i - 1] + pVSendNum[i - 1];
    }
    if (ProcRank == 0) {
        for (int i = 0; i < Size; i++)
            for (int j = 0; j < Size; j++) {
                pTranspousedMatrix[i * Size + j] = pMatrix[j * Size + i];
            }
        std::copy(pVector, pVector + Size, pInitVector);
    }

    //Scatter the vector
    MPI_Scatterv(pInitVector, pVSendNum, pVSendInd, MPI_DOUBLE, pVector,
        pVSendNum[ProcRank], MPI_DOUBLE, 0, MPI_COMM_WORLD);
    // Scatter the rows
    MPI_Scatterv(pTranspousedMatrix, pSendNum, pSendInd, MPI_DOUBLE, pProcRows,
...
void ResultReplication(double* pProcResult, double* pResult, int Size,
    ...
    // in result vector
    int RestRows = Size * ProcNum; // Number of rows, that haven�t been distributed yet
    int i; // Loop variable
    //Alloc memory for temporary objects
    ...
    double* gazeredResults = new double[Size * ProcNum];
    //Gather the whole result vector on every processor
    MPI_Gatherv(pProcResult, pReceiveNum[ProcRank], MPI_DOUBLE, gazeredResults,
    ...
    if (ProcRank == 0) {
        for (int i = 0; i < ProcNum; i++) {
            for (int j = 0; j < Size; j++) {
                ...
                pResult[j] += gazeredResults[i * Size + j];
            }
        }  
    }
...
void ParallelResultCalculation(double* pProcRows, double* pVector,
    ...
    for (i = 0; i < RowNum; i++) {
        for (j = 0; j < Size; j++) {
            ...
            pProcResult[j] += pProcRows[i * Size + j] * pVector[i];
        }
    }
}
