...
void StartSerialCalculation(
    ...
    if (ProcRank == 0) {
        ...
        SerialResultCalculation(pMatrix, pVector, pSerialResult, Size);
        finish = clock();
        SerialFinish = (finish - start) / double(CLOCKS_PER_SEC);
...
int main(int argc, char* argv[]) {
    ...
    //StartSerialCalculation(pMatrix, pVector, pResult, Size, pProcRows, pProcResult, RowNum);

    StartParallelCalculation(pMatrix, pVector, pResult, Size, pProcRows, pProcResult, RowNum);
